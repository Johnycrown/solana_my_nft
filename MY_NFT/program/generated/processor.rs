...
use crate::generated::errors::MyNftError;
use crate::generated::instructions::MyNftInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = MyNftInstruction::unpack(data)?;

        match instruction {
			MyNftInstruction::Mint(args) => {
			...
			MyNftInstruction::Transfer => {
				msg!("Instruction: Transfer");
				Self::process_transfer(program_id, accounts)
			}
			MyNftInstruction::Burn => {
...
/// - short_description: [String] 
	pub fn process_mint(
		...
		if fee_payer_info.is_signer != true {
			return Err(MyNftError::InvalidSignerPermission.into());
		}
		...
		if *gem_info.key != gem_pubkey {
			return Err(MyNftError::NotExpectedAddress.into());
		}
		...
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(MyNftError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(MyNftError::InvalidAccountLen.into());
		}
